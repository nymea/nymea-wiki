...
<br />
The reason for using ids instead of device/thing names is because the id is persitent. The user might 
still rename a thing and existing scripts will still continue work. If we'd use the device's name here,
the scripts would be more fragile and break easily.
...
<br />
A DeviceSate is required to have `deviceId` and `stateName` in order ot identify a thing. In addition to that a
